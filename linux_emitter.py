...
modifier = {'Control': (uinput.KEY_LEFTCTRL,),
...
class Emitter():
    ...
    """
    def emit_keycode(self, mod, datum):
        with self.emit_lock:
            e = self.event_dict.get(datum, None)
            modkey = modifier.get(mod, None)
            if e is not None:
                if modkey is None:
                    if len(e) == 1:
                        if verbose:
                            print('emit 1:', datum, file=stderr)
                        if not silent:
                            self.device.emit_click(e[0])
                    elif len(e) == 2:
                        if verbose:
                            print('emit 2:', datum, file=stderr)
                        if not silent:
                            self.device.emit_combo(e)
                    else:
                        print(
                            '\nwrong tuple size (not 1 or 2) for event for',
                            '|{0}|.'.format(datum), file=stderr)
                else:
                    if verbose:
                        print('emitting {0} + {1}'.format(mod, datum), file=stderr)
                    if not silent:
                        # unpack tuple, prepend modifier, repack as tuple
                        self.device.emit_combo(tuple(chain(modifier[mod], datum)))
            else:
                print('\nkeycode |{0}| not found.'.format(datum), file=stderr)
...
class EmitterService(rpyc.Service):
    ...
    def exposed_NS_keycode(self, mod, keycode):
